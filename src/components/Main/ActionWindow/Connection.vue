        ...
        <div class="row span justify-content-start">
          ...
          <div class="col-lg-6">
            ...
            <div class="row" v-if="showAdvanced">
              <div class="col-lg-9" style="margin-top: 5px">
                <text-input
                  v-model="connectionDetails.amqpHostName"
                  text="AMQP Hostname"
                  :disabled="disabled"
                />
              </div>
              <div class="col-md-3">
                <text-input
                  class="space"
                  v-model="connectionDetails.amqpPort"
                  text="AMQP Port"
                  :disabled="disabled"
                />
              </div>
          ...
          <div class="col-lg-2">
            ...
            <select-input
              ...
              "
              v-model="connectionDetails.vhost"
          ...
          <div class="col-lg-2 d-flex justify-content-center">
            ...
            >
              <span
                v-if="$store.getters.getCurrentTab.status['isLoading']"
              ...
              />
              {{
                $store.getters.getCurrentTab.status["isLoading"]
                  ? "Loading..."
                  : $store.getters.getCurrentTab.status["isActive"]
                  ? "stop"
                  : "start"
              }}
            </button>
...
export default {
  ...
  methods: {
    queryAmqp() {
      this.$ipc
        ...
        .then((value) => {
          if (
            ...
            (this.connectionDetails.amqpHostName === "" ||
              this.tmpHostName === this.connectionDetails.amqpHostName)
          ) {
        ...
        .catch(() => {
          this.connectionDetails.amqpHostName = "localhost";
          this.connectionDetails.amqpPort = "5672";
          this.$toast.error(
            `could not connect to rabbit at ${this.connectionDetails.apiHostName}:${this.connectionDetails.apiPort}`,
            {
              timeout: 2000,
            }
          );
    ...
    parseConnectionString() {
      let conn = this.connectionString
        .replace("http://", "")
        .replace("amqp://", "")
        .split("/")[0];
      let tmp = conn.split("@");
  ...
  computed: {
    ...
    isValid: {
      get() {
        ...
        let isUserValid =
          tab.connection.username !== "" && tab.connection.password;
        let isUriValid =
          tab.connection.amqpHostName !== "" && tab.connection.amqpPort !== "";
        let isConnectionValid =
          isUserValid && isUriValid && tab.connection.vhost !== "";
        let isOptionValid =
          tab.selectedOption.name !== "" && tab.selectedOption.type !== "";
  ...
  mounted() {
    ...
    this.queryAmqp();
    this.minimizeConnectionString();
    this.$store.commit("changeActionDetails", this.actionType);
  },
};
</script>
