<template>
  <div class="margins">
    <div class="row span justify-content-start">
      <label class="text-light">Path: </label>
      <label class="text-light" v-text="selectedFolder"></label>
    </div>
    <div class="row span justify-content-start">
      ...
      <div
        ...
        style="max-width: 300px; text-align: start"
      >
        <select-input
          :disabled="disabled"
      ...
      >
        <div class="publishClass">
          <number-input
            :disabled="disabled"
            v-model="actionDetails.rateDetails.rate"
            :min="0"
            text="Message Rate (Hz)"
          />
          <number-input
            :disabled="disabled"
            v-model="actionDetails.rateDetails.multiplier"
            :min="1"
            :max="4"
  ...
  computed: {
    ...
    selectedOption: {
      ...
      set(value) {
        ...
        let selectedOption =
          this.$store.getters.getCurrentTab.tmpLists.optionList[index];
        this.$store.commit("setTabValue", {
          key: "selectedOption",
